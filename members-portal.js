(function () {
  ...
  function addStyles() {
    ...
    s.textContent = [
      ...
      '.ll-modal .ll-error { font-size:13px; margin-bottom:16px; text-align:center; min-height:18px; }',
  ...
  function openModal(clerk) {
    ...
    bg.innerHTML =
      '<div class="ll-modal">' +
        ...
        '<div class="ll-field"><input type="email" id="ll-email" placeholder="Email" autocomplete="email"></div>' +
        '<div class="ll-field"><input type="password" id="ll-password" placeholder="Password" autocomplete="current-password"><button type="button" class="ll-toggle-pw" id="ll-toggle-pw">Show</button></div>' +
        '<div class="ll-remember"><label><input type="checkbox" id="ll-remember"> Remember me</label><a href="#" id="ll-forgot">Forgot Password?</a></div>' +
    ...
    document.getElementById('ll-forgot').addEventListener('click', function(e) {
      ...
      if (!email) { errEl.style.color = '#c0392b'; errEl.textContent = 'Please enter your email first.'; return; }
    ...
    document.getElementById('ll-submit').addEventListener('click', function() {
      ...
      errEl.textContent = '';
      if (!email || !password) { errEl.style.color = '#c0392b'; errEl.textContent = 'Please enter your email and password.'; return; }
      btn.disabled = true; btn.textContent = 'SIGNING IN...';
      clerk.client.signIn.create({ identifier: email, password: password })
        .then(function(result) {
          if (result.status === 'complete') {
            clerk.setActive({ session: result.createdSessionId }).then(function() { window.location.href = GC.redirectAfterLogin; });
          } else {
            errEl.style.color = '#c0392b'; errEl.textContent = 'Sign in incomplete. Contact ' + GC.supportEmail;
            btn.disabled = false; btn.textContent = 'SIGN IN';
          }
        })
        .catch(function(err) {
          var msg = (err.errors && err.errors[0] && err.errors[0].message) || 'Incorrect email or password.';
          errEl.style.color = '#c0392b'; errEl.textContent = msg;
          btn.disabled = false; btn.textContent = 'SIGN IN';
        });
    });
  ...
  function showDenied(clerk) {
    ...
    document.getElementById('ll-sb').addEventListener('click', function(e) { e.preventDefault(); openModal(clerk); });
  }

  function setupToggle() {
    var allFields = document.querySelectorAll('.form-item');
    var partnerFields = [];
    allFields.forEach(function(f) { if ((f.textContent || '').match(/Partner/i)) partnerFields.push(f); });
    ...
    document.addEventListener('click', function(e) {
      ...
      if (txt === 'couple' || txt === 'couples') { partnerFields.forEach(function(f) { f.style.display = ''; }); }
      else if (txt === 'individual') { partnerFields.forEach(function(f) { f.style.display = 'none'; }); }
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
      ...
      if (window.Clerk) {
        ...
        clerk.load().then(function() {
          ...
          if (isLogin()) {
            ...
            window.__llOpenModal = function() { openModal(clerk); };
            document.addEventListener('click', function(e) {
              var el = e.target.closest('a, button');
              if (!el) return;
              var txt = (el.textContent || '').trim().toUpperCase();
              if (txt === 'ENTER') { e.preventDefault(); e.stopPropagation(); openModal(clerk); }
            }, true);
            return;
          }
          ...
          if (isGated()) {
            if (user) {
              ...
              if (nav && !document.getElementById('ll-signout-btn')) {
                var btn = document.createElement('button');
                btn.id = 'll-signout-btn';
                btn.textContent = 'SIGN OUT';
                btn.addEventListener('click', function() { clerk.signOut().then(function() { window.location.href = '/'; }); });
                nav.appendChild(btn);
              }
            } else {
              document.body.classList.remove('loulou-checking');
            ...
            clerk.addListener(function() {
              if (!clerk.user) {
                var sb = document.getElementById('ll-signout-btn');
                if (sb) sb.remove();
                showDenied(clerk);
              }
            });
    ...
    setTimeout(function() { clearInterval(t); if (isGated() && document.body.classList.contains('loulou-checking')) { document.body.classList.remove('loulou-checking'); showDenied(null); } }, 15000);
  });
